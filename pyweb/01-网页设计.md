...
- 元数据：`meta`
  ...
  - 加载类型：`http-equiv`
    ...
    - `Set-Cookie`：若网页过期，则网页存在本地的 cookies 会被自动删除
...
- 预格式化：`pre`，维持源代码样式，若不设置，HTML 输出时，会把空格，回车等符号都忽略。
- 辅助内容
  - 引用：`blockquote`
...
- 若 `margin`是负数，则里面的元素会和外面 `margin`重合。
...
- 相对单位
  - 相对于另一个长度值而言：如 `em`或 `rem`。例如， `em`是基于元素的字体大小。若用它来设置 `font-size`属性，它是相对于**父元素**的 `font-size`而言的。
...
<style>
  ...
  .penguin-top {
    top: 10%;
    left: 25%;
    /* 第二个参数为 fallback 值，若变量失效了的话会使用这个 */
