...
![connectivity](images/ch12/conn.png)
...
![ER-degree](images/ch12/er_degree.png)
...
- 当$p<(1 -ɛ)\ln n/n$，则$G(n, p)$中的一个图几乎必有孤立节点，因而它是不连通的。
- 当$p>(1 + ɛ)\ln n/n$，则$G(n, p)$中的一个图几乎一定是连通的。
...
![ws-experiment](images/ch12/ws_exp.png)
...
![Dijkstra](images/ch12/dijkstra.png)
...
他们测量每个节点的度并计算${\rm PMF}(k)$，即节点度为$k$的比例。然后他们在双对数标度上绘制${\rm PMF}(k)$与$k$的关系。这些曲线可用一条直线拟合，至少对于$k$的较大数值；故他们得出结论，这些分布是重尾的。
...
其中，${\rm PMF}(k)$是度为$k$的节点的比例， $α$是一个参数，符号$∼$表示当$k$增加时，${\rm PMF}(k)$渐近于$k^{−α}$。

对两边取对数，得到，

$$
\log{\rm PMF}(k) ∼ −α\log k
$$

![heavy tailed distribution](images/ch12/heavy_tailed_dist.png)
...
$$
\log{\rm CCDF}(x) = -α (\log x − \log x_m)
$$

故，若分布服从幂定律，在双对数刻度上，CCDF 是斜率为 $-α$的直线。

![CDF && CCDF](images/ch12/cdf_ccdf.png)

### 3.4. HK 模型

[^1]: E.N. Gilbert (1959). "Random Graphs". Annals of Mathematical Statistics 30 (4): 1141–1144. ".doi:10.1214/aoms/1177706098"
[^2]: Erdős, P.; Rényi, A. (1959). "On Random Graphs. I". Publicationes Mathematicae 6: 290–297.
[^3]: Erdős, P.; Rényi, A. (1960). "On the evolution of random graphs". Magyar Tudományos Akadémia Matematikai Kutató Intézetének Kőzleményei [Publications of the Mathematical Institute of the Hungarian Academy of Sciences] 5: 17–61.
[^4]: Barabási, Albert-László; Albert, Réka (October 1999). "Emergence of scaling in random networks" (PDF). Science. 286 (5439): 509–512.
