...
- 邻居：细胞自动机的演化规则是局部的，对于指定细胞的状态进行更新时只需要知道其临近细胞的状态。某一细胞状态更新时要搜索的空间域叫做该细胞的邻居。
- 邻居的划分：在四方网格划分下的二维细胞自动机的邻居通常有以下三种形式：

![neighbor](images/ch2/neighbor.png)
...
![boundary](images/ch2/boundary.png)
...
- IV（complex）：出现复杂的局部结构，或者说是局部的混沌，其中有些会不断地传播。
...
在 GoL 中，每个单元格的下一个状态取决于其当前状态和活动邻居的数量。这种规则可概括为 DSRO（Death, Survival, Reproduction, Overpopulation）。若细胞的生前邻居数量$<D$或$>O$，则细胞死亡；若细胞的生前邻居数量$≤ S$，则细胞存活；若细胞的生前邻居数量$≥ R$且$≤ O$，则细胞复活。
...
Langton 对 CA 的另一个重要贡献是提出了$λ$法则，通过该法则，我们可对任一个 CA 的可预测性进行分类。
...
可用互相关（cross-correlation）来更快地更新 CA。一个数组$a$，与窗口$w$的互相关是一个新的数组$c$：

$$
c_k = ∑_{n= 0}^{N -1} a_{n +k} ⋅w_n
$$

其中，$N$为元素个数，窗口$w$中，$c$的每个元素是其邻居之和，具体的操作是类似"滑动点积"。

![cross correlation](images/ch2/cross_correlation.png)
...
class Cell1D:
    ...
    def draw(self, frame=[0, None], mode=None):
        if mode == 'random':
            self.start_random()
        elif mode == 'string':
            ...
            self.start_middle()
        self.loop(self.n_rows-1)
        a = self.array[:, frame[0]:frame[1]]
        plt.subplots()
